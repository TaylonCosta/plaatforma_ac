...
def SubmicaoCreate(request):
    ...
    todos_certificados = Certificado.objects.filter(usuario=user)
...
def VizualizarErros(request, pk):
    ...
    respostas = SubmissaoResponse.objects.filter(submissao__pk=int(pk))
...
class SubmissaoSecretariaList(DashboardListView):
    ...
    def get_queryset(self):
        qs = Submicao.objects.filter(status=1)
        return qs
